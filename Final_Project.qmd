...
```
This dataset contains `r nrow(opt)` participants. Of wchich, `r sum(opt$Group == "C")` were in the control group and `r sum(opt$Group == "T")` were in the Treatment group.
...
opt_table <- tbl_summary(data = opt,
                         by = group_cat,
...
Read @tbl-one for the descriptive statistics of the study participants.
...
```{r}
pd_median<- function(x,ref_vec = opt$V3.PD.avg){
  med_v3 <- stats::median(ref_vec, na.rm = TRUE)
  ifelse(
    is.na(x), NA,
    ifelse(x >= med_v3, "At/Above median V3 PD", "Below median V3 PD")
  )
}

print("V3 PD category vs cohort median for the first 10 participants:")

pd_median (opt$V3.PD.avg[1:10])

```

6. Inline R code - 1 pulling a statistic from a table and 1 printing something else. 
