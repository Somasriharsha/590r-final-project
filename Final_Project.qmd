...
```{r}
yes_rate <- function(x, success = "Yes", include_missing = FALSE) {
  n_yes   <- sum(x == success, na.rm = TRUE)
  n_denom <- if (include_missing) length(x) else sum(!is.na(x))
  paste(success, ": ", n_yes, " of ", n_denom, sep = "")
}

yes_rate(opt$Diabetes)
```
